...
### Added
* Add `stableDeviceId` to `getDeviceInfo()` across all platforms
* Add Dart convenience API `getStableDeviceId()`

### Changed
* Persist `stableDeviceId` per platform (Android: SharedPreferences, iOS/macOS: Keychain, Windows: Registry, Linux: XDG config file, Web: localStorage)
